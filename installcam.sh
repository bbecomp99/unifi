...
if [[ "${script_option_custom_url}" != 'true' ]]; then
  ...
  if wget "${wget_progress[@]}" -qO "$unifi_temp" "${unifi_video_download_url}" --no-check-certificate; then echo -e "${GREEN}#${RESET} Successfully downloaded UniFi Video version ${unifi_clean}! \\n"; else echo -e "${RED}#${RESET} Failed to download UniFi Video version ${unifi_clean}...\\n"; abort; fi
else
